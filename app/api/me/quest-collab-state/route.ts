import { NextResponse } from "next/server";
import { withTransaction } from "@/lib/db";
import { requireTesterId } from "@/lib/session";
import { errorJson } from "@/lib/apiResponses";
import { ensureSharedQuestsSchema } from "@/lib/sharedQuestsDb";

/**
 * Single round-trip for pending collab invites + membership collab quests.
 * Same payload shape as GET /quest-invites + GET /quest-collabs combined.
 */
export async function GET(_req: Request) {
  const auth = await requireTesterId(_req);
  if (!auth.ok) return auth.response;

  try {
    const { invites, membershipRows } = await withTransaction(
      async (tx) => {
        await ensureSharedQuestsSchema(tx);

        const invRes = await tx(
          `
          select
            i.id,
            i.quest_id::text as quest_id,
            q.data->>'title' as quest_title,
            q.data as quest_data,
            m.display_name as inviter_name,
            i.created_at
          from shared_quest_invites i
          join shared_quest_quests q on q.id = i.quest_id
          join shared_quest_memberships m
            on m.quest_id = i.quest_id and m.tester_id = i.inviter_tester_id
          where i.invitee_tester_id = $1 and i.status = 'pending'
          order by i.created_at desc
          `,
          [auth.testerId],
        );

        const memRes = await tx<{
          quest_id: string;
          data: unknown;
          my_state: string;
          quest_row_status: string;
        }>(
          `
          select
            q.id::text as quest_id,
            q.data as data,
            m.state as my_state,
            q.status as quest_row_status
          from shared_quest_memberships m
          join shared_quest_quests q on q.id = m.quest_id
          where m.tester_id = $1
          and m.state in ('active', 'left')
          order by q.created_at desc
          `,
          [auth.testerId],
        );

        return {
          invites: invRes.rows,
          membershipRows: memRes.rows,
        };
      },
    );

    const nowMs = Date.now();

    const quests = membershipRows.map((r) => {
      const base =
        r.data && typeof r.data === "object"
          ? ({ ...(r.data as Record<string, unknown>) } as Record<string, unknown>)
          : {};
      const myState = r.my_state;
      const globalFailed = r.quest_row_status === "failed";

      if (myState === "left") {
        const ts =
          typeof base.failedAt === "number"
            ? base.failedAt
            : typeof base.completedAt === "number"
              ? base.completedAt
              : nowMs;
        return {
          ...base,
          collabQuest: true,
          myState: "left" as const,
          status: "failed" as const,
          failedAt: ts,
          completedAt: ts,
        };
      }

      if (globalFailed) {
        const ts =
          typeof base.failedAt === "number"
            ? base.failedAt
            : typeof base.completedAt === "number"
              ? base.completedAt
              : nowMs;
        return {
          ...base,
          collabQuest: true,
          myState: myState as "active" | "left",
          status: "failed" as const,
          failedAt: ts,
          completedAt: ts,
        };
      }

      return {
        ...base,
        collabQuest: true,
        myState: myState as "active" | "left",
      };
    });

    return NextResponse.json({ ok: true, invites, quests });
  } catch (e) {
    console.error("GET /api/me/quest-collab-state failed:", e);
    return errorJson("server_error", 500);
  }
}
